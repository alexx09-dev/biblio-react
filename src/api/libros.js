...
export const crearLibro = async (datos) => {
  ...
  if (!datos.autor?.trim())  throw new Error('El autor es obligatorio.')
...
// [FASE 7A] Marca o desmarca un libro como favorito
export const toggleFavorito = async (id, esFavorito) => {
  const response = await api.patch(`/${id}/favorito`, { es_favorito: esFavorito })
  return response.data
}
